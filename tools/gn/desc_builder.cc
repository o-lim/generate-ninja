...
//   "command" : "command for action targets",
//   "interpreter" : "interpreter for action targets",
...
#define CONFIG_VALUE_ARRAY_HANDLER(name, type)                        \
  if (what(#name)) {                                                  \
    ...
    CONFIG_VALUE_ARRAY_HANDLER(asmppflags, std::string)
    ...
    CONFIG_VALUE_ARRAY_HANDLER(cppflags, std::string)
    CONFIG_VALUE_ARRAY_HANDLER(cppflags_c, std::string)
    CONFIG_VALUE_ARRAY_HANDLER(cppflags_cc, std::string)
    CONFIG_VALUE_ARRAY_HANDLER(cppflags_objc, std::string)
    CONFIG_VALUE_ARRAY_HANDLER(cppflags_objcc, std::string)
  ...
  std::unique_ptr<base::DictionaryValue> BuildDescription() {
    ...
    if (target_->output_type() == Target::ACTION ||
      ...
      if (target_->action_values().script().is_null()) {
        if (what(variables::kCommand))
          res->SetKey(variables::kCommand,
                      base::Value(target_->action_values().command().AsString()));
      } else {
        if (what(variables::kScript))
        res->SetKey(variables::kScript,
                    base::Value(target_->action_values().script().value()));

        if (target_->action_values().has_interpreter() && what(variables::kInterpreter))
          res->SetKey(variables::kInterpreter,
                      base::Value(target_->action_values().interpreter()));
      }
