...
bool PrintTarget(const Target* target,
  ...
  if (target->output_type() == Target::ACTION ||
    ...
    if (target->action_values().script().is_null()) {
      if (what.empty() || what == variables::kCommand) {
        PrintCommand(target, display_headers);
        found_match = true;
      }
    } else {
      ...
      if (target->action_values().has_interpreter() && (what.empty() || what == variables::kInterpreter)) {
        PrintInterpreter(target, display_headers);
        found_match = true;
      }
    }
