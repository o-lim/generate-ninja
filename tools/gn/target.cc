...
bool Target::IsLinkedBinary() const {
...
bool Target::GetOutputFilesForSource(const SourceFile& source,
  ...
  *computed_tool_type = Toolchain::TYPE_NONE;
  ...
  SourceFileType file_type = toolchain_->GetSourceFileType(source);
  if (file_type == SOURCE_UNKNOWN)
    return false;
  if (file_type == SOURCE_O || (file_type == SOURCE_LINK && IsLinkedBinary())) {
    // Object files and linker scripts just get passed to the output and not
    // compiled.
