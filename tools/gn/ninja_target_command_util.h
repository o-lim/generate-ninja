...
struct DefineWriter {
  explicit DefineWriter(const std::string& pre)
    : prefix(pre) {
    options.mode = ESCAPE_NINJA_COMMAND;
  }
  DefineWriter(const std::string& pre, EscapingMode mode, bool escape_strings)
      : prefix(pre), escape_strings(escape_strings) {
    options.mode = mode;
  }
  ...
  void operator()(const std::string& s, std::ostream& out) const {
    ...
    if (escape_strings) {
      ...
      EscapeStringToStream(out, prefix + dest, options);
      return;
    }
    EscapeStringToStream(out, prefix + s, options);
  }
...
struct IncludeWriter {
  IncludeWriter(const std::string& prefix, PathOutput& path_output)
    : prefix_(prefix), path_output_(path_output) {}
