...
```
### <a name="cmd_meta"></a>**gn meta**: List target metadata collection results.
```
...
```
  ...
  The `output_conversion` variable specifies the format to write the
  ...
  trigger a metadata collection walk based on the dependencies of the target
  and the optional values of the `rebase` and `walk_keys` variables. See
  `gn help metadata`.
